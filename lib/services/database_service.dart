...
import 'package:untitled3/models/nutrition_log_model.dart';
...
class DatabaseService {
  ...
  Future<String> addExercise(Exercise exercise) async {
    if (exercise.id != null) {
      await _db.collection('exercises').doc(exercise.id).set(exercise.toMap(), SetOptions(merge: true));
      return exercise.id!;
    } else {
  ...
  Stream<List<Exercise>> getExercises() {
    return _db.collection('exercises').snapshots().map((snapshot) =>
        snapshot.docs.map((doc) => Exercise.fromMap(doc.data(), doc.id)).toList());
  }
  ...
  Future<String> createProgram(Program program) async {
    DocumentReference ref = await _db.collection('programs').add(program.toMap());
    return ref.id;
  }
  ...
  Future<void> updateProgram(Program program) async {
    ...
    if (program.createdForClientId != null && program.id != null) {
      ...
      if (doc.exists) {
        ...
        await _db.collection('programs').doc(program.id).collection('versions').doc(oldVersion.toString()).set(oldData);
        ...
        await _db.collection('programs').doc(program.id).set(newProgram.toMap(), SetOptions(merge: true));
        return;
      }
    }
    await _db.collection('programs').doc(program.id).set(program.toMap(), SetOptions(merge: true));
  }
  ...
  Future<void> createProgramCopy(String originalId, String newClientId, DateTime startDate) async {
    // 1. Fetch Original
    ...
    final daysSnapshot = await _db.collection('programs').doc(originalId).collection('days').get();
    ...
    NotificationService.showLocalNotification('Program Assigned', 'A new program has been assigned from template.');
  }

  Future<void> applyGlobalUpdate(Program publicProgram, List<WorkoutDay> newDays) async {
    // 1. Update the public program itself
    await updateProgram(publicProgram);
    
    // 2. Find all copies and update them
    final copiesSnapshot = await _db.collection('programs')
    ...
    for (var doc in copiesSnapshot.docs) {
      ...
      final existingDays = await _db.collection('programs').doc(copyId).collection('days').get();
      final batch = _db.batch();
      for (var dayDoc in existingDays.docs) {
        batch.delete(dayDoc.reference);
      }
      for (var day in newDays) {
        final newDayRef = _db.collection('programs').doc(copyId).collection('days').doc();
        batch.set(newDayRef, day.toMap());
      }
      await batch.commit();

      // Notify Client
      NotificationService.notifyClientOfProgramUpdate(copyData['assignedClientId'] ?? '', publicProgram.name);
    }
  }
  ...
  Stream<List<Program>> getProgramTemplates(String coachId) {
    return _db
        ...
        .snapshots()
        .map((snapshot) =>
            snapshot.docs.map((doc) => Program.fromMap(doc.data(), doc.id)).toList());
  }
  ...
  Stream<List<Program>> getCoachPrograms(String coachId) {
    return _db
        ...
        .where('coachId', isEqualTo: coachId)
        .snapshots()
        .map((s) => s.docs.map((doc) => Program.fromMap(doc.data(), doc.id)).toList());
  }
  ...
  Future<String> claimPublicProgram(String publicProgramId, String clientId) async {
    // 1. Fetch the public program
    final publicProgramDoc = await _db.collection('programs').doc(publicProgramId).get();
    if (!publicProgramDoc.exists) throw Exception('Public program not found');
    
    final publicProgram = Program.fromMap(publicProgramDoc.data()!, publicProgramDoc.id);
    ...
    final newProgramRef = await _db.collection('programs').add(newProgram.toMap());
    
    // 3. Copy workout days (templates)
    final daysSnapshot = await _db.collection('programs').doc(publicProgramId).collection('days').get();
    ...
    for (var dayDoc in daysSnapshot.docs) {
      ...
      final exerciseLogsSnapshot = await dayDoc.reference.collection('exercise_logs').get();
  ...
  Future<void> assignProgramToClient(String programId, String clientId, DateTime startDate) async {
    ...
    NotificationService.showLocalNotification('New Program Assigned', 'Go to "My Training" to see your new plan.');
  }
  ...
  Stream<List<Program>> getClientPrograms(String clientId) {
    return _db
        ...
        .orderBy('status') // Active first, then replaced/completed if we add those statuses
        .snapshots()
        .map((snapshot) =>
            snapshot.docs.map((doc) => Program.fromMap(doc.data(), doc.id)).toList());
  }
  ...
  Future<void> deleteProgramCompletely(String programId) async {
    ...
    final children = await _db.collection('programs').where('parentProgramId', isEqualTo: programId).get();
    ...
    final logsSnapshot = await _db.collection('logs').where('programId', isEqualTo: programId).get();
    for (var doc in logsSnapshot.docs) {
      batch.delete(doc.reference);
    }

    // 3. Delete all days and their subcollections (exercise_logs)
    final daysSnapshot = await _db.collection('programs').doc(programId).collection('days').get();
    for (var dayDoc in daysSnapshot.docs) {
      // Delete exercise logs inside the day
      final exLogsSnapshot = await dayDoc.reference.collection('exercise_logs').get();
  ...
  Stream<double> getProgramProgress(String programId) {
    ...
    return Stream.fromFuture(_db.collection('programs').doc(programId).collection('days').get())
      ...
      return _db
          ...
          .snapshots()
          .map((logsSnap) => (logsSnap.docs.length / totalDays).clamp(0.0, 1.0));
    });
  }

  // To make it reactive to NEW logs, we can use a more combined approach
  Stream<double> getLiveProgramProgress(String programId) {
    final daysStream = _db.collection('programs').doc(programId).collection('days').snapshots();
    final logsStream = _db.collection('logs').where('programId', isEqualTo: programId).snapshots();
    ...
    return daysStream.asyncMap((daysSnap) async {
      ...
      final logsSnap = await _db.collection('logs').where('programId', isEqualTo: programId).get();
  ...
  Future<double> getProgramProgressStatic(String programId) async {
    final days = await _db.collection('programs').doc(programId).collection('days').get();
    final logs = await _db.collection('logs').where('programId', isEqualTo: programId).get();
  ...
  Stream<List<ExerciseLog>> getExerciseLogs(String programId, String dayId) {
    return _db
        ...
        .snapshots()
        .map((snapshot) =>
            snapshot.docs.map((doc) => ExerciseLog.fromMap(doc.data())).toList());
  }
  ...
  Stream<List<NutritionLog>> getNutritionLogs(String clientId) {
    return _db
        ...
        .snapshots()
        .map((snap) => snap.docs.map((doc) => NutritionLog.fromMap(doc.data(), doc.id)).toList());
  }
  ...
  Stream<List<WorkoutDay>> getProgramDays(String programId) {
    return _db
        ...
        .snapshots()
        .map((snapshot) =>
            snapshot.docs.map((doc) => WorkoutDay.fromMap(doc.data(), doc.id)).toList());
  }

  Future<void> logExerciseProgress(String programId, String dayId, ExerciseLog log, String clientId) async {
    ...
    if (progDoc.exists) {
      ...
      NotificationService.cancelWorkoutReminder(p.assignedClientId ?? '', p.name, p.currentDay);
    }
    ...
    if (progDoc.exists) {
      final p = Program.fromMap(progDoc.data()!, progDoc.id);
      NotificationService.notifyCoachOfCompletion(p.coachId, 'Client', p.name);
    }

    // 2. Check if all exercises for the day are terminal
    final dayDoc = await _db.collection('programs').doc(programId).collection('days').doc(dayId).get();
  ...
  Future<void> _unlockNextDay(String programId, int completedWeek, int completedDay) async {
    final progDoc = await _db.collection('programs').doc(programId).get();
    ...
    if (prog.currentDay < maxDayInWeek) {
      ...
      await _db.collection('programs').doc(programId).update({'currentDay': prog.currentDay + 1});
      ...
      final clientName = "Client"; // In a real app, we'd fetch the client's name or pass it in
      ...
      NotificationService.notifyProgramCompletion(coachId, clientName, prog.name);
    }
  }

  Future<void> skipExercise(String programId, String dayId, String exerciseName, int version) async {
    ...
    await logExerciseProgress(programId, dayId, log, ""); // ClientId handled via programId context
  }
  ...
  Stream<List<WorkoutLog>> getClientLogs(String clientId) {
    return _db
        ...
        .snapshots()
        .map((snapshot) =>
            snapshot.docs.map((doc) => WorkoutLog.fromMap(doc.data(), doc.id)).toList());
  }
  ...
  Future<void> updateWorkoutFeedback(String logId, String feedback) async {
    ...
    NotificationService.showLocalNotification('New Coach Feedback', 'Your coach has left feedback on your recent workout.');
  }

  // --- Aggregate Stats ---
  Stream<Map<String, dynamic>> getClientStats(String clientId) {
    return getClientLogs(clientId).map((logs) {
      if (logs.isEmpty) return {'consistency': 0, 'sessions': 0, 'totalWeight': 0.0, 'trends': []};
      
      double totalWeight = 0;
      Map<DateTime, double> dailyVolume = {};
      Map<String, List<Map<String, dynamic>>> exerciseTrends = {}; // ExerciseName -> [{date, weight}]
      ...
      final trends = sortedDates.asMap().entries.map((e) => {'day': e.key, 'volume': dailyVolume[e.value]}).toList();
  ...
  Future<void> sendMessage(ChatMessage message) async {
    ...
    NotificationService.handleIncomingMessage(message.senderId == 'coach' ? 'Coach' : 'Client', message.text);
  }

  Future<void> sendBroadcastMessage(String coachId, List<String> clientIds, String text) async {
    final batch = _db.batch();
  ...
  Stream<NutritionPlan?> getActiveNutritionPlan(String clientId) {
    return _db
        ...
        .map((snapshot) => snapshot.docs.isNotEmpty
            ? NutritionPlan.fromMap(snapshot.docs.first.data(), snapshot.docs.first.id)
            : null);
  }

  Future<void> saveNutritionPlan(NutritionPlan plan) async {
    final docRef = plan.id == null
        ? _db.collection('users').doc(plan.clientId).collection('nutrition_plans').doc()
        : _db.collection('users').doc(plan.clientId).collection('nutrition_plans').doc(plan.id);

    final planToSave = plan.copyWith(updatedAt: DateTime.now());
    await docRef.set(planToSave.toMap(), SetOptions(merge: true));
    
    // Notify Client
    NotificationService.notifyClientOfNutritionUpdate(plan.clientId);
  }
  ...
  Future<void> saveWeeklyCheckIn(WeeklyNutritionCheckIn checkin) async {
    final docRef = checkin.id == null
        ? _db.collection('users').doc(checkin.clientId).collection('nutrition_checkins').doc()
        : _db.collection('users').doc(checkin.clientId).collection('nutrition_checkins').doc(checkin.id);

    await docRef.set(checkin.copyWith(updatedAt: DateTime.now()).toMap(), SetOptions(merge: true));
  }

  Stream<WeeklyNutritionCheckIn?> getLatestWeeklyCheckIn(String clientId, DateTime weekStartDate) {
    return _db
        .collection('users')
        .doc(clientId)
        ...
        .snapshots()
        .map((snapshot) => snapshot.docs.isNotEmpty
            ? WeeklyNutritionCheckIn.fromMap(snapshot.docs.first.data(), snapshot.docs.first.id)
            : null);
  }

  Stream<List<WeeklyNutritionCheckIn>> getWeeklyCheckinsForWeek(String clientId, DateTime weekStartDate) {
    return _db
        .collection('users')
        .doc(clientId)
        .collection('nutrition_checkins')
        ...
        .snapshots()
        .map((snapshot) =>
            snapshot.docs.map((doc) => WeeklyNutritionCheckIn.fromMap(doc.data(), doc.id)).toList());
  }

  Stream<List<WeeklyNutritionCheckIn>> getWeeklyCheckInHistory(String clientId) {
    return _db
        .collection('users')
        .doc(clientId)
        .collection('nutrition_checkins')
