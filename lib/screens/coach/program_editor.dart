...
class ProgramEditor extends StatefulWidget {
  ...
  const ProgramEditor({super.key, required this.coachId, this.preSelectedClientId, this.programToEdit});
...
class _ProgramEditorState extends State<ProgramEditor> {
  ...
  void _loadClients() {
    _clientsSubscription = _dbService.getClients(widget.coachId).listen((clients) {
        if (mounted) {
          setState(() {
            _clients = clients;
          });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(widget.programToEdit == null ? 'Program Builder' : 'Edit Program'),
      ),
      body: Stack(
        children: [
          Column(
              ...
              Expanded(
                child: ListView(
                  ...
                  children: [
                    ...
                    _buildDaysList(),
                    const SizedBox(height: 16),
                    _buildSecondaryButton('Add Workout Day to Week $_selectedWeek', _addNewDay, icon: Icons.add),
              ...
              child: const Center(
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 16),
                    Text('Saving Program...', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                  ],
                ),
              ),
            ),
      ...
      bottomNavigationBar: Container(
        ...
        child: ElevatedButton(
          ...
          child: Text(_isLoading ? 'SAVING...' : (widget.programToEdit == null ? 'CREATE PROGRAM' : 'SAVE CHANGES'), style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
        ),
      ),
    );
  }
  ...
  Widget _buildWeekPickerHeader() {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          const Text('PHASE / WEEK', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: AppTheme.mutedTextColor)),
          TextButton.icon(
            onPressed: _duplicateCurrentWeek,
            icon: const Icon(Icons.copy, size: 14),
            label: const Text('Duplicate to Next Week', style: TextStyle(fontSize: 12)),
          ),
        ],
      ),
    );
  ...
  void _duplicateCurrentWeek() {
    if (_selectedWeek >= _weeks) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Cannot duplicate to next week (limit reached).')));
      return;
    }
    final nextWeek = _selectedWeek + 1;
    final currentWeekDays = _days.where((d) => d.week == _selectedWeek).toList();
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Week $_selectedWeek duplicated to Week $nextWeek')));
  }
    ...
    return Column(
      ...
      children: [
        ...
        Padding(
          ...
          child: Row(
            children: [
              Text('Duration (Weeks):', style: TextStyle(color: Colors.white.withOpacity(0.6))),
              const SizedBox(width: 12),
              SizedBox(
                width: 60,
                child: TextField(
                  ...
                  decoration: const InputDecoration(
                    ...
                    border: UnderlineInputBorder(borderSide: BorderSide(color: AppTheme.primaryColor)),
                  ),
                  onChanged: (val) {
                    ...
                    if (n != null) {
                      if (n > 20) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('Maximum duration is 20 weeks'), duration: Duration(seconds: 1)),
                        );
                        _durationController.text = '20';
                        _durationController.selection = TextSelection.fromPosition(const TextPosition(offset: 2));
                        setState(() {
                          _weeks = 20;
                        });
                      } else if (n > 0) {
        ...
        if (widget.preSelectedClientId != null)
               ...
               children: [
                 ...
                 Expanded(
                   child: Column(
                     crossAxisAlignment: CrossAxisAlignment.start,
                     children: [
                       const Text('Creating Personal Program For:', style: TextStyle(color: AppTheme.mutedTextColor, fontSize: 12)),
                       const SizedBox(height: 4),
                       Text(
                         _clients.firstWhere((c) => c.uid == widget.preSelectedClientId, orElse: () => AppUser(uid: '', email: '', name: 'Loading...', role: UserRole.client)).name,
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: Theme.of(context).cardTheme.color,
              borderRadius: BorderRadius.circular(8),
              border: Border.all(color: _selectedType == null ? Colors.orangeAccent.withOpacity(0.5) : Colors.white10),
            ),
            child: DropdownButtonHideUnderline(
              child: DropdownButton<ProgramType>(
                value: _selectedType,
                hint: const Text('Select Program Type', style: TextStyle(color: Colors.orangeAccent)),
                isExpanded: true,
                    ...
                    setState(() {
                      if (newValue != null) {
                        _selectedType = newValue;
                         if (_selectedType == ProgramType.personal && widget.preSelectedClientId != null) {
                           _assignedClientId = widget.preSelectedClientId;
            ...
            Container(
              ...
              child: DropdownButtonHideUnderline(
                child: DropdownButton<String>(
                  ...
                  hint: const Text('Select Client', style: TextStyle(color: AppTheme.mutedTextColor)),
                  isExpanded: true,
                  dropdownColor: AppTheme.surfaceColor,
          ...
          child: Column(
            ...
            children: [
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('Day ${day.day}: ${day.muscleGroup}'.toUpperCase(), style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, letterSpacing: 1.2, color: AppTheme.mutedTextColor)),
                  IconButton(
              ...
              _buildSecondaryButton('Add Exercise', () => _addExerciseToDay(indexInAll), icon: Icons.add_circle_outline),
            ],
          ),
        );
      }).toList(),
            ...
            child: Column(
              ...
              children: [
                ...
                if (e.targetWeight != null && e.targetWeight!.isNotEmpty)
                   Text('Target: ${e.targetWeight}', style: const TextStyle(color: AppTheme.primaryColor, fontSize: 11, fontWeight: FontWeight.bold)),
                if (e.duration != null && e.duration!.isNotEmpty)
                   Text('Duration: ${e.duration}', style: const TextStyle(color: Colors.lightBlueAccent, fontSize: 11)),
                if (e.totalReps != null)
                   Text('Total Reps: ${e.totalReps}', style: const TextStyle(color: Colors.white70, fontSize: 11)),
                if (e.volume != null)
                   Text('Volume: ${e.volume}', style: const TextStyle(color: Colors.white70, fontSize: 11)),
                if (e.note != null && e.note!.isNotEmpty)
                   Text('Note: ${e.note}', style: const TextStyle(color: Colors.white60, fontSize: 10, fontStyle: FontStyle.italic)),
                if (e.altExercise != null && e.altExercise!.isNotEmpty)
                  Text('Alt: ${e.altExercise}', style: const TextStyle(color: Colors.orangeAccent, fontSize: 10, fontStyle: FontStyle.italic)),
              ],
            ),
          ),
          IconButton(
            icon: const Icon(Icons.delete_outline, size: 20, color: Colors.redAccent),
            onPressed: () => setState(() => _days[dayIndex].exercises.removeAt(exIndex)),
          ),
        ],
      ),
    );
  ...
  List<Widget> _buildExercisesWithSupersets(WorkoutDay day, int dayIndex) {
    ...
    for (var i = 0; i < day.exercises.length; i++) {
      ...
      if (e.supersetLabel != null && e.supersetLabel!.isNotEmpty && e.supersetLabel != currentSuperset) {
        ...
        list.add(Padding(
          ...
          child: Row(
            children: [
              ...
              Text(currentSuperset!.toUpperCase(), style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: AppTheme.primaryColor)),
            ],
          ),
        ));
  ...
  Widget _buildSecondaryButton(String label, VoidCallback onTap, {IconData? icon}) {
        ...
        builder: (context, setDialogState) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              DropdownButtonFormField<String>(
                value: muscleGroup,
                items: ['Quadriceps', 'Glutes_Hamstrings', 'Calves', 'Chest', 'Back', 'Shoulders', 'Triceps', 'Biceps', 'Abs', 'Other']
                    .map((g) => DropdownMenuItem(value: g, child: Text(g.replaceAll('_', ' ')))).toList(),
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                setState(() {
                  _days.add(WorkoutDay(week: week, day: day, muscleGroup: muscleGroup, exercises: []));
                });
      ...
      builder: (context) => Container(
        ...
        onSelected: (selectedExercises) {
          setState(() {
            ...
            final existing = _days[dayIndex].exercises.where((oldEx) =>
              selectedExercises.any((newEx) => newEx.name == oldEx.name)).toList();

            // Add new exercises from the picker that aren't in the day yet
            final newlyAdded = selectedExercises.where((newEx) =>
              !_days[dayIndex].exercises.any((oldEx) => oldEx.name == newEx.name)).toList();
  ...
  void _saveProgram() async {
    if (_nameController.text.isEmpty || _days.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please enter a name and add at least one day.')));
      return;
    }

    if (_selectedType == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please select a program type.')));
      return;
    }

    if (_selectedType == ProgramType.personal && _assignedClientId == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please select a client for this personal program.')));
      return;
    }

    final isTemplate = _selectedType == ProgramType.template;
    final isPublic = _selectedType == ProgramType.allUsers;
    final assignedId = _selectedType == ProgramType.personal ? _assignedClientId : null;
    ...
    final program = Program(
      ...
      startDate: assignedId != null ? (widget.programToEdit?.startDate ?? DateTime.now()) : null,
      createdForClientId: assignedId ?? widget.programToEdit?.createdForClientId,
    );

    // If editing a PUBLIC program, ask for update mode
    if (widget.programToEdit != null && widget.programToEdit!.isPublic && isPublic) {
       ...
       try {
         if (applyGlobally) {
           ...
           final String? targetClientId = await _showClientPickerForDuplication();
           if (targetClientId == null) {
             setState(() => _isLoading = false);
             return;
           }
           await _dbService.createProgramCopy(program.id!, targetClientId, DateTime.now());
         }
    ...
    try {
      ...
      if (mounted) {
        Navigator.pop(context);
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(widget.programToEdit == null ? 'Program created!' : 'Program updated!')));
      }
    } catch (e) {
      if (mounted) {
        setState(() => _isLoading = false);
  ...
  Future<bool?> _showGlobalUpdateDialog() async {
    return showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Text('Save Public Program', style: TextStyle(color: Colors.white)),
        content: const Text(
        ...
        actions: [
          TextButton(
            ...
            child: const Text('APPLY GLOBALLY', style: TextStyle(color: AppTheme.primaryColor, fontWeight: FontWeight.bold)),
          ),
          TextButton(
            onPressed: () => Navigator.pop(context, false),
            child: const Text('DUPLICATE TO PERSONAL', style: TextStyle(color: Colors.orangeAccent)),
          ),
          TextButton(
  ...
  Future<String?> _showClientPickerForDuplication() async {
     ...
     return showDialog<String>(
       ...
       builder: (context) => StatefulBuilder(
         builder: (context, setDialogState) => AlertDialog(
           backgroundColor: AppTheme.surfaceColor,
           title: const Text('Select Client', style: TextStyle(color: Colors.white)),
           content: DropdownButton<String>(
             value: selectedId,
             hint: const Text('Choose Client', style: TextStyle(color: AppTheme.mutedTextColor)),
             isExpanded: true,
             dropdownColor: AppTheme.surfaceColor,
             items: _clients.map((c) => DropdownMenuItem(value: c.uid, child: Text(c.name))).toList(),
...
class _ExercisePicker extends StatefulWidget {
  ...
  const _ExercisePicker({required this.onSelected, required this.initialExercises});
      ...
      children: [
        ...
        Padding(
          ...
          child: Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              const Text('Add Exercises', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
              TextButton.icon(
                onPressed: () {
                  Navigator.push(
                    context,
                    ...
                    if (mounted) Navigator.pop(context); // Close picker after creation
                  });
                },
        ...
        Padding(
              ...
              builder: (context, snapshot) {
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final isSelected = _selectedExercises.any((selected) => selected.id == ex.id);
                    return CheckboxListTile(
                      title: Text(ex.name, style: const TextStyle(fontWeight: FontWeight.bold)),
                      subtitle: Text(ex.muscleGroup.replaceAll('_', ' '), style: const TextStyle(fontSize: 12, color: AppTheme.mutedTextColor)),
                      value: isSelected,
                      ...
                      onChanged: (bool? value) {
                        setState(() {
                          if (value == true) {
                            ...
                            _selectedExercises.removeWhere((selected) => selected.id == ex.id);
                          }
                        });
                      },
                    );
          ...
          Padding(
            ...
            child: ElevatedButton(
              onPressed: _selectedExercises.isEmpty ? null : () {
