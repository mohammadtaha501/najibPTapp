...
class _WorkoutViewState extends State<WorkoutView> {
  ...
  void initState() {
    ...
    for (int i = 0; i < widget.workoutDay.exercises.length; i++) {
      ...
      _logs[i] = List.generate(ex.sets ?? 3, (index) => SetLog(weight: 0, reps: 0));
    }
  }
  ...
  Widget _buildExerciseMedia(Exercise ex) {
    return GestureDetector(
      onTap: ex.videoUrl != null ? () => showExerciseGuidance(context, ex.name, ex.videoUrl!) : null,
      child: Container(
        height: 220,
        width: double.infinity,
        decoration: BoxDecoration(
          color: Colors.black,
          image: ex.videoUrl != null ? null : const DecorationImage(
            image: NetworkImage('https://images.unsplash.com/photo-1534438327276-14e5300c3a48?q=80&w=1000'), // Placeholder for missing media
            fit: BoxFit.cover,
            opacity: 0.6,
          ),
        ),
  ...
  List<Widget> _buildExerciseList(BuildContext context) {
    ...
    for (int i = 0; i < widget.workoutDay.exercises.length; i++) {
      ...
      if (exercise.supersetLabel != null && exercise.supersetLabel!.isNotEmpty && exercise.supersetLabel != currentSuperset) {
        ...
        list.add(Padding(
          ...
          child: Row(
            children: [
              ...
              Text(currentSuperset!.toUpperCase(), style: const TextStyle(fontWeight: FontWeight.bold, color: AppTheme.primaryColor, letterSpacing: 1.2)),
            ],
          ),
        ));
      } else if (exercise.supersetLabel == null || exercise.supersetLabel!.isEmpty) {
          ...
          children: [
            ...
            Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(exercise.name, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
                  ...
                  if (exercise.targetWeight != null && exercise.targetWeight!.isNotEmpty) ...[
                    ...
                    Container(
                      ...
                      child: Text('TARGET: ${exercise.targetWeight}', style: const TextStyle(color: AppTheme.primaryColor, fontSize: 11, fontWeight: FontWeight.bold)),
                    ),
                  ],
                  const SizedBox(height: 20),
                  ...List.generate(exercise.sets ?? 3, (setIndex) => _buildSetInputRow(i, setIndex)),
                  if (exercise.description != null && exercise.description!.isNotEmpty) ...[
                    ...
                    Container(
                      ...
                      child: Text('Cues: ${exercise.description}', style: const TextStyle(color: AppTheme.primaryColor, fontSize: 13, fontWeight: FontWeight.w500)),
                    ),
                  ],
                  if (exercise.altExercise != null && exercise.altExercise!.isNotEmpty) ...[
      ...
      child: Row(
        children: [
          CircleAvatar(
            ...
            child: Text('${setIndex + 1}', style: const TextStyle(fontSize: 10, color: AppTheme.primaryColor, fontWeight: FontWeight.bold)),
          ),
          const SizedBox(width: 8),
          ...
          TextButton(
            ...
            child: const Text('LOG', style: TextStyle(color: AppTheme.primaryColor, fontSize: 10, fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    );
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            Text('REST: $_restRemaining s', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
            const Spacer(),
            TextButton(
  ...
  Widget _buildVolumeBadge() {
    ...
    return Center(
      child: Container(
        ...
        child: Text('${total.toInt()} kg', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
      ),
    );
  }
        ...
        children: [
          SizedBox(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(hintText: hint, border: InputBorder.none, contentPadding: EdgeInsets.zero, fillColor: Colors.transparent),
            ),
          ),
  ...
  void _saveLog() async {
    ...
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error logging workout: $e')));
    }
  }
}
